<template>
  <v-main class="list" style="margin: auto;">
    ...
    <div v-if="isWideScreen" style="margin-bottom: 7.5%">
      <h3 style="float:left">Kendaraan</h3>
      <v-spacer></v-spacer>
      <v-btn
        small
        color="primary"
            ...
            <v-list width="90" class="py-0" style="margin-top: 20px">
              <v-list-item>
                <v-list-item-content>
                  ...
                  >
                  <v-list-item-title style="color: #000000; margin-top: 10px"
                    ><v-btn small @click="deleteHandler(item.id_kendaraan)"
                      ><v-icon color="#C94141"
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card height="20%" style="background: #196b4d; border-radius: 4px 4px 0px 0px">
        <v-card-title>
          <!-- <v-spacer></v-spacer> -->
          <h3 style="font-size:20px; color:#ffffff">{{ formTitle }} Data Kendaraan</h3>
          <v-spacer></v-spacer>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Nomor Plat", value: "plat_kendaraan" },
        { text: "", value: "actions" },
      ],
  ...
  methods: {
    ...
    readData() {
      var url = this.$api + "/kendaraanRead";
      this.$http
        .get(url, {
          headers: {
    ...
    readDataRemove() {
      var url = this.$api + "/kendaraanRemove";
      this.$http
        .get(url, {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
        ...
        .then((response) => {
          this.kendaraansR = response.data.data;
        });
    },
    save() {
    ...
    updateTemp() {
      var url = this.$api + "/kendaraanTemp";
      this.load = true;
      this.$http
        .put(url, {
          headers: {
    ...
    update() {
      let newData = {
        role_kendaraan: this.form.nama_role,
        nama_kendaraan: this.form.nama_kendaraan,
        tanggal_lahir_kendaraan: this.form.tanggal_lahir_kendaraan,
        email_kendaraan: this.form.email_kendaraan,
        no_telp_kendaraan: this.form.no_telp_kendaraan,
      };
    ...
    //non aktif data kendaraan
    deleteData() {
      var url = this.$api + "/kendaraanDelete/" + this.deleteId;
      this.load = true;
      this.$http
        .put(url, {
          headers: {
    ...
    editHandler(item) {
      ...
      this.form.role_kendaraan = item.role_kendaraan;
      this.form.nama_kendaraan = item.nama_kendaraan;
      this.form.tanggal_lahir_kendaraan = item.tanggal_lahir_kendaraan;
      this.form.email_kendaraan = item.email_kendaraan;
      this.form.no_telp_kendaraan = item.no_telp_kendaraan;
      this.dialog = true;
    },
    deleteHandler(id) {
      this.deleteId = id;
  ...
  mounted() {
    localStorage.setItem("menu", "Kendaraan");
