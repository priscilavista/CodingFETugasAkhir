          ...
          <v-menu offset-y style="float: left">
            <template v-slot:activator="{ on, attrs }">
              <span v-bind="attrs" v-on="on" style="cursor: pointer">
                <v-tooltip top>
                  <template v-slot:activator="{ on, attrs }">
                    <v-icon v-bind="attrs" v-on="on" v-if="item.status_sppbe=='A'" color="primary" @click="editHandler(item)" style="margin-right: 15px; font-size: 20px">
                      mdi-pencil
                    </v-icon>
                  </template>
                  <span>Ubah Data</span>
                ...
                <v-tooltip top>
                  <template v-slot:activator="{ on, attrs }">
                    <v-icon v-bind="attrs" v-on="on" v-if="item.status_sppbe=='A'" @click="deleteHandler(item)" color="error" style="font-size: 20px">
                      mdi-store-remove
                    </v-icon>
                  </template>
                ...
                <v-tooltip top>
                  <template v-slot:activator="{ on, attrs }">
                    <v-icon  v-bind="attrs" v-on="on" v-if="item.status_sppbe=='D'" @click="deleteHandler(item)" color="success" style="font-size: 20px">
                      mdi-store-check
                    </v-icon>
                  </template>
